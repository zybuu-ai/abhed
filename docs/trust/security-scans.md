...
| file | Rule | Triage |
|---|---|---|
| `internal/tools/syntax.go` (`parsesPython`) | G204 — subprocess launched with a variable | **Accepted.** The program is the real file behind the `python3` on the path, resolved through its symlinks and refused if it lies inside the workspace or any extra root, which the agent can write; invoking the real file also bypasses any virtualenv. The arguments are fixed: `-I -S -c` with a constant script that calls `compile()`, which parses and does not execute; `-S` stops `site`, and so `.pth` files, from loading, and `-I` stops the environment, user site packages and the current directory from being read. The environment is empty, the working directory is the temp directory, the content goes in on standard input, and the process is killed after five seconds. `TestPythonIsCompiledNotRun`, `TestAnInterpreterInsideTheWorkspaceIsNeverRun` and `TestSitePackagesDoNotLoad` fail if any of this regresses |
