...
| High | server/pty.go:198 | `exec.CommandContext(ctx, "bash", "-c", req.Command)` — a line typed into the workbench terminal, when no `Sandbox` function is configured | Accepted with reason — the same fallback as bash.go:151, for the person at the keyboard rather than the model. The line has already passed the policy engine as a `bash` call (`ManualAuthorizeTyped`), is recorded, and normally runs through the sandbox's own command builder; the fallback is the same `Sandbox == nil` deployment concern flagged below |
